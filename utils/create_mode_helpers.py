...
from utils.input_helpers import ask_until_valid_with_quit, non_empty_factory
from domain.errors import QuitOperationError

T = TypeVar("T")

def ask_new_text(label: str) -> str:
    return ask_until_valid_with_quit(f"{label}: ", non_empty_factory(label))

def ask_new_date(label: str) -> date:
    # parse_date already checks format and not-in-the-past
    return ask_until_valid_with_quit(f"{label} (YYYY-MM-DD): ", parse_date)

def ask_new_priority(default: str = "medium"):
    def _p(s: str):
        s = (s or default).strip()
        return choose_priority(s)
    return ask_until_valid_with_quit(
